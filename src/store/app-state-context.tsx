...
//   useState,
// } from "react";
// import { ISO3166_3letter_country_codes } from "../types/common/common.types";
...
//   userid: string;
//   email?: string;
//   countryCode: ISO3166_3letter_country_codes;
...
//   countryCode: "IND",
//   nativeLanguage: "en",
//   favouriteLanguages: new Set<string>(["te", "hi"]),
//   theme: "light",
//   availableLanguages: {},
//   dummyLocations: [
...
export interface AppContextInterface {
  ...
  // New State for Bottom Navigation Visibility
...
export const defaultCtx: AppContextInterface = {
  ...
  translations: {
    en: {
      ...
      CONFIRM_PASSWORD: "Confirm password",
      LOGIN: "Login",
      WE_KEEP_YOU_SAFE: "We keep you safe! Your password is encrypted and inaccessible to anyone except you!",
      SOMETHING_WENT_WRONG_TRY_AGAIN: "Something went wrong! Please try again",
      LETS_SETUP: "Let's setup",
      CREATE_PROFILE: "Create your profile here!",
      KEEP_ME_SIGNED_IN: "Keep me signed in",
  ...
  // Default visibility of Bottom Navigation
  showBottomNavigation: true,
  setShowBottomNavigation: () => {},
};

const AppCtx = createContext<AppContextInterface>(defaultCtx);
export const AppCtxUpdater = createContext<Dispatch<SetStateAction<AppContextInterface>>>(() => {});

export const AppCtxProvider: React.FC<PropsWithChildren> = ({ children }) => {
  const [contextState, setContextState] = useState<AppContextInterface>(() => {
    const ctxStringInStorage = localStorage.getItem("TRANSLANGO_APP_CTX");
    if (ctxStringInStorage) {
      const ctx: AppContextInterface = JSON.parse(ctxStringInStorage);
  ...
  return (
    <AppCtx.Provider value={{ ...contextState, setShowBottomNavigation: (show) => setContextState((prev) => ({ ...prev, showBottomNavigation: show })) }}>
      <AppCtxUpdater.Provider value={setContextState}>{children}</AppCtxUpdater.Provider>
    </AppCtx.Provider>
  );
};
